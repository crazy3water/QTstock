...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        for index_data_name in self.index_data_names:
            # self.sma = bt.indicators.MovingAverageSimple(eval(index_data_name),
    ...
    def get_buy_strategy(self,code_i):
        if self.day_count > 10:
            if self.macd_ps[code_i][0]<0 and self.macd_ps[code_i][-2] < self.macd_ps[code_i][-1] and self.macd_ps[code_i][0] > self.macd_ps[code_i][-1] \
                    and self.rsis[code_i][0] < 40 and self.rsis[code_i][-1]<self.rsis[code_i][-2] and self.rsis[code_i][-1] < self.rsis[code_i][0]:
                return True
            else:
                return False
    ...
    def next(self):
        ...
        pa = False
        if self.MACD_RSI_signal.l.buy_signal:
            pa = True
            s = self.MACD_RSI_signal.l.buy_signal
        ...
        for i in range(len(self.params.codes)):
            if self.get_sell_strategy(i):
                num = self.positionsbyname["code_"+self.params.codes[i]].size
    ...
    def stop(self):
        ...
        for code in self.params.codes:
            self.log("目前仓位：\n"+str(self.positionsbyname["code_"+code]), doprint=True)
